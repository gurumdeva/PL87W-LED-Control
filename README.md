...
이 앱은 Apple Developer ID 로 서명·공증되지 않은 오픈소스 빌드라서, 그냥 dmg 를 받아
드래그하면 macOS Gatekeeper 가 "손상되었기 때문에 열 수 없습니다" 메시지로 차단한다.
아래 세 가지 설치 옵션 모두 이 차단을 자동 우회한다. **편한 걸 골라서** 한 가지만
실행하면 된다.

### 옵션 1 — 터미널 한 줄 (가장 빠름, 추천)

```bash
curl -fsSL https://github.com/gurumdeva/PL87W-LED-Control/raw/main/install.sh | bash
```

다운로드 + Applications 로 복사 + quarantine 우회까지 자동.

### 옵션 2 — dmg 의 자동 설치기 (터미널 안 쓰는 사용자)
...
`Install.command` 가 .app 을 Applications 로 복사하고 quarantine 속성을 제거한다.
처음 한 번은 `Install.command` 도 quarantine 경고가 뜨면 우클릭 → "열기" 로 우회.

### 옵션 3 — 수동 드래그 + xattr 명령
