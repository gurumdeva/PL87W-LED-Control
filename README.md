...
macOS 는 인터넷에서 다운로드한 앱에 quarantine 속성을 자동으로 부착해,
첫 실행 시 "손상되었기 때문에 열 수 없습니다" 메시지로 차단할 수 있다.
**아래 옵션 중 편한 걸 하나만** 골라 실행하면 이 quarantine 을 자동으로 처리한다.

### 🍺 Homebrew (가장 추천)

[Homebrew](https://brew.sh) 가 설치되어 있다면 한 줄로 끝:

```sh
brew install --cask gurumdeva/pl87w/pl87w-led-control
```

업데이트는 `brew upgrade --cask pl87w-led-control`, 제거는
`brew uninstall --cask pl87w-led-control`. Homebrew tap 정의는
[gurumdeva/homebrew-pl87w](https://github.com/gurumdeva/homebrew-pl87w).
...
다운로드 + Applications 복사 + quarantine 우회까지 자동.

### dmg + GUI 자동 설치기 (터미널 안 쓰는 사용자)

1. [Releases](../../releases) 에서 `PL87W_LED_Control.dmg` 다운로드
2. dmg 더블 클릭으로 마운트
3. dmg 안의 **`Install.command`** 더블 클릭
4. "설치 완료" 알림에서 "지금 열기" 클릭

처음 한 번은 `Install.command` 자체도 quarantine 경고가 뜨면 우클릭 → "열기" 로 우회.

### 수동 드래그 + xattr 명령 (참고)

원리 이해하고 직접 처리하고 싶을 때:

1. dmg 마운트 → `.app` 을 Applications 로 드래그
2. "손상됨" 메시지가 뜨면 터미널에서:
   ```bash
   xattr -dr com.apple.quarantine "/Applications/PL87W LED Control.app"
   ```
