...
`PL87W LED Control.app` 을 `Applications` 폴더로 드래그해 넣는다.

### "손상되었기 때문에 열 수 없습니다" 메시지가 뜨면

이 앱은 Apple Developer ID 로 정식 서명·공증되지 않은 오픈소스 빌드라서, 인터넷에서
다운로드한 직후 macOS Gatekeeper 가 quarantine 속성을 자동으로 붙인다. macOS Sonoma /
Sequoia 부터는 이 상태에서 "손상됨" 으로 즉시 차단된다. 터미널에서 한 줄로 해제:

```bash
xattr -dr com.apple.quarantine "/Applications/PL87W LED Control.app"
```

명령 실행 후 다시 더블 클릭하면 정상 실행된다. (한 번만 하면 됨)
